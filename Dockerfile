...
RUN apk --update upgrade && apk add python py-pip ca-certificates openssl haproxy && \
    rm -rf /var/cache/apk/* && \
...
RUN apk --update add --virtual build-deps python-dev build-base linux-headers && \
    pip install docker-py termcolor 'psutil >=2.2.1,<3.0' jinja2 python-etcd peewee && \
    apk del build-deps && \
    rm -rf /var/cache/apk/*
